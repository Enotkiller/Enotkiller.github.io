...
async def cmd_start(message: Message):
    ...
    if db.data_weekly() <= 5 and db.time_float() <= 14.50 and message.from_user.id != 5376094724:
        await message.answer(f"{days[db.data_weekly() - 1]}\nТекущее время: {db.time()}\n{"Текущая" if not db.peremena() else "Будет"} пара: {db.para()}\nСтатус: {(("Отпустили" if db.otmena_now() == 1 else "Перемена" if db.peremena() else "Идёт") if db.para() != "Пары нет" else "Ничего нет") if message.from_user.id != 7250542929 else (("Отпустили" if db.otmena_now() == 1 else "Перемена" if db.peremena() else "Не идёт, а летит") if db.para() != "Пары нет" else "Ничего нет")}\nСсылка: {db.get_url()}")
